...
正式运行前先确认本次计划扫描数量；不再隐含默认 50 只：

```bash
python run_live.py --limit 100
```
...
`--tickers` 和 `--limit` 二选一。直接运行 `python run_live.py` 时，工具会先询问“今天准备扫描多少只股票”，确认后才开始请求数据。每次计划数量只是本次批次，不代表完整股票池已经完成；中断时会保留 `output/workflow_plan.json` 和已有续跑标记。5 只只是路演演示参数，不是产品的扫描上限。
...
如果希望“由 Agent 启动一次就完成抓取和日报”，使用 `run_live.py`。它会先确认本次计划数量，再抓取 ND100，按 CSV 内的真实行情日改名，再调用完整日报入口；同日已有正式日报时自动复用，不覆盖。若电脑休眠或 Agent 重启，入口会优先接管续跑标记和半成品，不重复抓取。
...
`--cache-only` 只读取已有缓存，不会因为历史缓存的文件修改时间较早就请求 API；缺失的数据会明确记录为“数据不足”。入口还会检查批次之间是否重复，以及五榜单、SKDJ 是否复用了与本次 ND100 输入不同范围的旧报告。发现范围不一致时会停止发布，不会生成混批日报。

它会先读取 CSV 内的真实 `日线_数据截至`，再检查同日正式报告；完整同日报告会复用，半成品会从已完成阶段继续，过期锁会自动接管，不会覆盖历史结果。通过门禁后才运行五榜、T9、SKDJ、状态链和日报整理。失败可用 `--retry 2 --retry-delay 120` 重试。
...
`run_daily.py` 只接受完整的 ND100 输入。50/102 等 partial universe 会在进入五榜单、T9、SKDJ 前被阻断；需要续跑时，先补齐剩余批次，再把全部批次一起传入。
